...
def denoise_ffdnet(Y, sigma, color=1):
    ...
    if 'clip' in model_name:
        ...
        use_clip = False
    model_pool = './ffdnet-main/pnp/model_zoo'  
...
data_dir="./adv_data"  #adv
# data_dir="./clean_samples" #clean
list1 =[]
def data_adv(data_dir):
    ...
    summ=0
    models=ResNet18()
    models = torch.load("./resnet18.pth")
    z=0
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
...
#                 os.remove(img_path)
    for img_name in path_list:
        ...
        # resnet = models.to(device)       
        # resnet.eval()             
        # out = resnet(batch_t)
        # out.size()
        # with open('/root/data1/code/GitHubcode/cifar-10/cifar.txt') as f:
        ...
        tool1=transforms.ToTensor()
        img_tensor=tool1(img)
        #print(img.shape)
        ...
        img_t=img_t(img_tensor).to(device)
